...
tic
for i =1:LL
    x0 = fun_TrainData('g',N,1,R0,2,1,1);
    X = fun_TrainData('g',N,L,R0,2,1,1);
    RNX = abs(fun_NSCMN(X));
    RX = abs(fun_SCMN(X));
%     Rx0=fun_SCMN(x0);
...
%     R_MAM_iter = fun_iterMAMH1(MAM,RNX);
...
%     error_R_MAM_iter(i) = norm(R_MAM_iter-R0,'fro')/norm(R0,'fro');
%     
