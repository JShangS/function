...
%%%%%%%%%%%训练数据产生%%%%%%%%%%%%%%
    ...
    %%%%%% CLGLRT
    Tclglrt(i) = fun_iGLCGLRT(lambda,mu,R_KA,R_SCMN,x0,s);
end
toc
% close(h)
...
%         waitbar(((m-1)*MonteCarloPd+i)/length(SNRout)/MonteCarloPd,h,sprintf([num2str(((m-1)*MonteCarloPd+i)/length(SNRout)/MonteCarloPd*100),'%%']));
        ...
        %%%%%% GLCGLRT
        Tclglrt = fun_iGLCGLRT(lambda,mu,R_KA,R_SCMN,x0,s);
        %%%判断%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
...
str=['Pd_iGLCGLRT_',num2str(n),'K','mu',num2str(mu),...
