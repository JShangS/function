...
cost = -1/numCase * groundTruth(:)' * log(p(:)) + lambda/2 * sum(softmaxTheta(:) .^ 2);
softmaxThetaGrad = -1/numCase * (groundTruth - p) * a{depth+1}' + lambda * softmaxTheta;
d = cell(depth+1);

 d{depth+1} = -(softmaxTheta' * (groundTruth - p)) .* a{depth+1} .* (1-a{depth+1});
%d{depth+1} = -(softmaxTheta' * (groundTruth - p)) .* z{depth+1} .* (1-z{depth+1});

for layer = (depth:-1:2)
   d{layer} = (stack{layer}.w' * d{layer+1}) .* a{layer} .* (1-a{layer});
    %d{layer} = (stack{layer}.w' * d{layer+1}) .* z{layer} .* (1-z{layer});
end
...
%   stackgrad{layer}.b = (1/numClasses) * sum(d{layer+1}, 2);
  ...
  stackgrad{layer}.w = (1/numCase) * d{layer+1} * a{layer}';
  stackgrad{layer}.b = (1/numCase) * sum(d{layer+1}, 2);
end
% -------------------------------------------------------------------------
